...
class FraudPredictor:
    ...
    async def predict(self, data: Dict[str, Any]) -> Dict[str, Any]:
        ...
        # Add centrality measures to dataframe
        for key, value in centrality_measures.items():
            df[key] = value.astype("float32")  # Ensure float32 type

        # Create DMatrix for prediction
        dmatrix = self.preprocessor.create_dmatrix(df[Config.FEATURE_COLUMNS])

        # Make prediction
        prediction = self.model.predict_proba(dmatrix)[0]
